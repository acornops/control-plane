...
function snapshotMatches(agent: AgentDefinition, server: Awaited<ReturnType<typeof listAgentMcpServers>>[number]): boolean {
  ...
  const projectInstallation = (
    item: Pick<typeof installation, 'id' | 'name' | 'url' | 'enabled' | 'credentialMode' | 'revision'>
  ...
  try {
    ...
    if (!existing) {
      ...
      const effectiveTools = tools.map((tool) => {
        const current = existing.tools.find((candidate) => candidate.name === tool.name);
        return current ? { ...tool, enabled: current.enabled } : tool;
      });
      const catalogChanged = !toolCatalogMatches(existing.tools, effectiveTools);
