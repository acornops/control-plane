import { streamGatewayJsonPatch } from './checkpoint-gateway.js';
import { randomUUID } from 'node:crypto';
import { withInsightsCapacity, assertInsightsExecution } from '../insights-capacity.js';
...
async function processJob(job: Awaited<ReturnType<typeof repo.claimDueTargetInsightsCheckpointJobs>>[number]): Promise<void> {
  ...
  await withInsightsCapacity(job, async (execution) => {
  ...
  await assertInsightsExecution(execution);
  ...
  const terminalStatus = await withTransaction(async (client) => {
    await assertInsightsExecution(execution, client);
