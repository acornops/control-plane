...
export async function listClusterFindings(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    const workspaceId = toSingleParam(req.params.workspaceId);
    const clusterId = toSingleParam(req.params.clusterId);
    const access = await requireClusterAccess(req, res, workspaceId, clusterId);
    ...
    const page = await repo.listClusterSnapshotFindings(clusterId, {
      limit: parseBoundedLimit(req.query.limit),
      cursor,
      q,
