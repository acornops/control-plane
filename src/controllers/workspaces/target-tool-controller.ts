...
import {
  requireTargetMcpServerCreate,
  requireTargetMcpServerUpdate,
  requireTargetMcpToolSettings,
  respondMissingMcpCapability,
  respondMissingToolsCapability,
  targetMcpTrustBoundaryChanges
} from './target-mcp-helpers.js';
import {
  materializeInheritedTargetMcp
} from './target-mcp-defaults.js';

export { listTargetMcpCatalog } from './target-mcp-list-controller.js';
...
export async function createTargetMcpServerForTarget(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const value = requireTargetMcpServerCreate(req.body, res);
    if (!value) return;
...
export async function updateTargetMcpServerForTarget(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const value = requireTargetMcpServerUpdate(req.body, res);
    if (!value) return;
    const materialization = await materializeInheritedTargetMcp({
      serverId,
      workspaceId,
      targetId,
      targetType: access.target.targetType,
...
export async function updateTargetMcpServerToolSettings(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const value = requireTargetMcpToolSettings(req.body, res);
    if (!value) return;
    ...
    const capability = requestedCapability === 'write' || requestedCapability === 'read'
      ...
      : existing.capability === 'write'
        ? 'write'
        : 'read';
    if (value.enabled && existing.source === 'mcp' && !existing.enabled && requestedCapability !== capability) {
      res.status(400).json({
        error: {
    ...
    await recordToolCatalogAudit(workspaceId, targetId, access.target.targetType, req.auth.userId, toolName, value.enabled, capability);
    res.status(200).json({
      ...updated,
      enabled: value.enabled
    });
