...
export async function syncTooling(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (requestedWorkspaceId || requestedTargetId || requestedTargetType) {
      ...
      await syncTargetBuiltInTools(requestedWorkspaceId, requestedTargetId, target.targetType);
    ...
    for (const registration of registrations) {
      await syncTargetBuiltInTools(registration.workspaceId, registration.targetId, registration.targetType);
      synced += 1;
    }
