...
describe('internal Agent Targets MCP bridge', () => {
  it('executes scoped target-awareness without requiring a bound target', async () => {
    ...
    const installation = {
      ...
      revision: 1,
      targetConstraints: { targetTypes: ['kubernetes' as const], targetIds: [] },
    ...
    const compiledAccessScope = compileWorkflowAccessScope({
      ...
      mappings: [],
      actor,
      approvedContextGrants: directWorkflow.capabilityPolicy.contextGrants
    });
    ...
    await callMcpTool({
      body: {
        name: 'list_targets',
        toolAlias: 'targets_list_targets',
        serverId,
    ...
    assert.equal(
      ...
      'AGENT_TARGETS_MCP_SCOPE_DENIED'
    );
    assert.equal(targetAgentCall.mock.callCount(), 0);
  });
});
