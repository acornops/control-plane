...
describe('control-plane metrics', () => {
  ...
  it('renders workspace-native outcomes with canonical bounded IDs and no correlation labels', () => {
    observeWorkspaceNativeToolCall('documents.create', 'success', 42);
    observeWorkspaceNativeToolCall('http.fetch.get', 'success', 125);
    observeWorkspaceNativeToolCall('unknown.dynamic.tool', 'failure', 75);
    const payload = renderControlPlaneMetrics();

    assert.match(payload, /control_plane_workspace_native_tool_calls_total\{[^}]*tool_id="documents\.create",outcome="success"[^}]*\} 1/);
    ...
    assert.match(payload, /control_plane_workspace_native_tool_call_duration_ms_bucket\{[^}]*tool_id="documents\.create",outcome="success",le="50"[^}]*\} 1/);
