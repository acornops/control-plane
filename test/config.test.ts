...
describe('parseAppConfig production validation', () => {
  ...
  it('requires the OAuth console URL to be a canonical public origin', () => {
    for (const MANAGEMENT_CONSOLE_BASE_URL of [
      ...
      assert.throws(
        () => parseAppConfig(productionEnv({ MANAGEMENT_CONSOLE_BASE_URL })),
        (error) => Boolean(fieldErrors(error).MANAGEMENT_CONSOLE_BASE_URL?.length)
      );
    }
  });
