...
function builtinServer(enabled = true, revision = 1) {
  return {
    ...
    target_type: 'agent',
    target_constraints: { target_types: ['kubernetes'], target_ids: [] },
...
describe('Agent Targets MCP management boundary', () => {
  it('rejects connection, deletion, individual tool, and no-op revision mutations', async () => {
    ...
    const toolChanged = await callController(patchTool, createRequest(
      { ...params(), toolName: 'list_targets' },
      { enabled: false }
    ));

    assert.equal(renamed.statusCode, 400);
    assert.equal(revisionOnly.statusCode, 400);
    assert.equal(removed.statusCode, 409);
    assert.equal(toolChanged.statusCode, 409);
    assert.ok([
      renamed,
      revisionOnly,
      removed,
