-- Complete greenfield control-plane schema.
-- This unreleased application intentionally has no incremental SQL migrations.
...
CREATE TABLE agent_definitions (
    ...
    CONSTRAINT agent_definitions_avatar_emoji_check CHECK (((char_length(avatar_emoji) >= 1) AND (char_length(avatar_emoji) <= 64))),
...
CREATE TABLE automation_dispatch_outbox (
    ...
    CONSTRAINT automation_dispatch_outbox_source_type_check CHECK ((source_type = 'workflow'::text)),
...
CREATE TABLE generated_documents (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    tool_call_id text,
    CONSTRAINT generated_documents_exactly_one_run_scope_check CHECK ((((workflow_execution_id IS NOT NULL) AND (workflow_run_id IS NOT NULL) AND (conversation_run_id IS NULL)) OR ((workflow_execution_id IS NULL) AND (workflow_run_id IS NULL) AND (conversation_run_id IS NOT NULL)))),
    CONSTRAINT generated_documents_provenance_check CHECK ((jsonb_typeof(provenance) = 'object'::text)),
    CONSTRAINT generated_documents_source_check CHECK ((jsonb_typeof(source) = 'object'::text)),
    CONSTRAINT generated_documents_source_size_bytes_check CHECK ((source_size_bytes >= 0))
);
...
CREATE TABLE runs (
    ...
    conversation_kind text DEFAULT 'target_chat'::text NOT NULL,
    agent_id text,
    agent_snapshot jsonb,
    compiled_access_scope jsonb,
    CONSTRAINT runs_assistant_references_array CHECK ((jsonb_typeof(assistant_references) = 'array'::text)),
    CONSTRAINT runs_conversation_binding_check CHECK ((((conversation_kind = 'target_chat'::text) AND (target_id IS NOT NULL) AND (agent_id IS NULL) AND (agent_snapshot IS NULL) AND (compiled_access_scope IS NULL)) OR ((conversation_kind = 'agent_chat'::text) AND (target_id IS NULL) AND (agent_id IS NOT NULL) AND (jsonb_typeof(agent_snapshot) = 'object'::text) AND (agent_snapshot @> jsonb_build_object('id', agent_id, 'workspaceId', workspace_id)) AND (jsonb_typeof(compiled_access_scope) = 'object'::text) AND (compiled_access_scope @> jsonb_build_object('agentId', agent_id, 'workspaceId', workspace_id)) AND (NOT (compiled_access_scope ?| ARRAY['workflowId'::text, 'executor'::text, 'selectedAgents'::text]))))),
    CONSTRAINT runs_conversation_kind_check CHECK ((conversation_kind = ANY (ARRAY['target_chat'::text, 'agent_chat'::text]))),
    ...
    CONSTRAINT runs_request_actor_provenance_check CHECK ((((request_actor_type = ANY (ARRAY['user'::text, 'system'::text])) AND (request_external_integration_link_id IS NULL) AND (request_external_integration_client_id IS NULL)) OR ((request_actor_type = 'external_integration'::text) AND (request_external_integration_link_id IS NOT NULL) AND (request_external_integration_client_id IS NOT NULL)))),
...
CREATE TABLE sessions (
    ...
    CONSTRAINT sessions_conversation_binding_check CHECK ((((conversation_kind = 'target_chat'::text) AND (target_id IS NOT NULL) AND (agent_id IS NULL) AND (preferred_access_mode IS NULL)) OR ((conversation_kind = 'agent_chat'::text) AND (target_id IS NULL) AND (agent_id IS NOT NULL) AND (preferred_access_mode IS NOT NULL) AND (origin = 'manual'::text) AND (linked_issue_id IS NULL) AND (linked_issue_lifecycle_version IS NULL) AND (auto_triage_write_mode IS NULL) AND (auto_triage_effective_tool_mode IS NULL) AND (auto_triage_confirmation_required IS NULL)))),
    CONSTRAINT sessions_conversation_kind_check CHECK ((conversation_kind = ANY (ARRAY['target_chat'::text, 'agent_chat'::text]))),
    CONSTRAINT sessions_origin_check CHECK ((origin = ANY (ARRAY['manual'::text, 'auto_triage'::text]))),
    CONSTRAINT sessions_preferred_access_mode_check CHECK (((preferred_access_mode IS NULL) OR (preferred_access_mode = ANY (ARRAY['read_only'::text, 'read_write'::text]))))
);
...
CREATE TABLE target_auto_triage_settings (
    ...
    namespace_include jsonb DEFAULT '[]'::jsonb NOT NULL,
    namespace_exclude jsonb DEFAULT '[]'::jsonb NOT NULL,
    include_cluster_scoped_issues boolean DEFAULT true NOT NULL,
    ...
    CONSTRAINT target_auto_triage_settings_namespace_exclude_check CHECK (((jsonb_typeof(namespace_exclude) = 'array'::text) AND (jsonb_array_length(namespace_exclude) <= 100) AND (jsonb_path_query_array(namespace_exclude, '$[*]?(@.type() == "string" && @ like_regex "^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$")'::jsonpath) = namespace_exclude))),
    CONSTRAINT target_auto_triage_settings_namespace_include_check CHECK (((jsonb_typeof(namespace_include) = 'array'::text) AND (jsonb_array_length(namespace_include) <= 100) AND (jsonb_path_query_array(namespace_include, '$[*]?(@.type() == "string" && @ like_regex "^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$")'::jsonpath) = namespace_include))),
...
CREATE SEQUENCE workflow_execution_events_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER SEQUENCE workflow_execution_events_id_seq OWNED BY workflow_execution_events.id;
...
CREATE TABLE workflow_executions (
    ...
    trigger_id text,
    occurrence_key text,
    origin_snapshot jsonb DEFAULT '{"kind": "manual", "label": "Manual", "schemaVersion": 1}'::jsonb NOT NULL,
    source_type text,
    source_id text,
...
CREATE TABLE workflow_webhook_deliveries (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT workflow_webhook_deliveries_status_check CHECK ((status = ANY (ARRAY['pending'::text, 'claimed'::text, 'delivered'::text, 'rejected'::text, 'failed'::text])))
);
...
CREATE TABLE workflow_webhook_events (
    ...
    occurred_at timestamp with time zone NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT workflow_webhook_events_payload_check CHECK ((jsonb_typeof(payload) = 'object'::text))
);
...
CREATE TABLE workflow_webhooks (
    ...
    last_run_id text,
    last_error text,
    CONSTRAINT workflow_webhooks_approved_context_grants_check CHECK ((jsonb_typeof(approved_context_grants) = 'array'::text)),
    CONSTRAINT workflow_webhooks_created_by_check CHECK ((jsonb_typeof(created_by) = 'object'::text)),
    CONSTRAINT workflow_webhooks_last_status_check CHECK (((last_status IS NULL) OR (last_status = ANY (ARRAY['dispatched'::text, 'failed'::text, 'auto_paused'::text, 'rejected'::text])))),
    CONSTRAINT workflow_webhooks_principal_check CHECK (((jsonb_typeof(principal) = 'object'::text) AND ((principal ->> 'type'::text) = 'user'::text) AND (COALESCE((principal ->> 'id'::text), ''::text) <> ''::text))),
    CONSTRAINT workflow_webhooks_status_check CHECK ((status = ANY (ARRAY['enabled'::text, 'paused'::text]))),
    CONSTRAINT workflow_webhooks_updated_by_check CHECK ((jsonb_typeof(updated_by) = 'object'::text))
);
...
CREATE TABLE workspace_default_skill_files (
    ...
    content_digest text NOT NULL,
    size_bytes integer NOT NULL,
    CONSTRAINT workspace_default_skill_files_size_bytes_check CHECK ((size_bytes >= 0))
);
...
CREATE TABLE workspace_defaults (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    enabled boolean DEFAULT true NOT NULL,
    CONSTRAINT workspace_defaults_available_in_check CHECK (((available_in = ARRAY['agents'::text]) OR (available_in = ARRAY['kubernetes'::text]) OR (available_in = ARRAY['virtual_machines'::text]) OR (available_in = ARRAY['agents'::text, 'kubernetes'::text]) OR (available_in = ARRAY['agents'::text, 'virtual_machines'::text]) OR (available_in = ARRAY['kubernetes'::text, 'virtual_machines'::text]) OR (available_in = ARRAY['agents'::text, 'kubernetes'::text, 'virtual_machines'::text]))),
    CONSTRAINT workspace_defaults_kind_check CHECK ((kind = ANY (ARRAY['mcp_server'::text, 'skill'::text])))
);
...
CREATE TABLE workspace_initial_default_skill_files (
    ...
    content_digest text NOT NULL,
    size_bytes integer NOT NULL,
    CONSTRAINT workspace_initial_default_skill_files_size_bytes_check CHECK ((size_bytes >= 0))
);
...
CREATE TABLE workspace_initial_defaults (
    ...
    CONSTRAINT workspace_initial_defaults_available_in_check CHECK (((available_in = ARRAY['agents'::text]) OR (available_in = ARRAY['kubernetes'::text]) OR (available_in = ARRAY['virtual_machines'::text]) OR (available_in = ARRAY['agents'::text, 'kubernetes'::text]) OR (available_in = ARRAY['agents'::text, 'virtual_machines'::text]) OR (available_in = ARRAY['kubernetes'::text, 'virtual_machines'::text]) OR (available_in = ARRAY['agents'::text, 'kubernetes'::text, 'virtual_machines'::text]))),
    CONSTRAINT workspace_initial_defaults_kind_check CHECK ((kind = ANY (ARRAY['mcp_server'::text, 'skill'::text])))
);
...
ALTER TABLE ONLY workflow_execution_events ALTER COLUMN id SET DEFAULT nextval('workflow_execution_events_id_seq'::regclass);
...
ALTER TABLE ONLY generated_documents
...
ALTER TABLE ONLY workflow_execution_events
    ADD CONSTRAINT workflow_execution_events_execution_dedupe_key UNIQUE (execution_id, dedupe_key);

ALTER TABLE ONLY workflow_execution_events
    ADD CONSTRAINT workflow_execution_events_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY workflow_webhook_deliveries
    ADD CONSTRAINT workflow_webhook_deliveries_event_id_webhook_id_key UNIQUE (event_id, webhook_id);

ALTER TABLE ONLY workflow_webhook_deliveries
    ADD CONSTRAINT workflow_webhook_deliveries_pkey PRIMARY KEY (id);

ALTER TABLE ONLY workflow_webhook_events
    ADD CONSTRAINT workflow_webhook_events_pkey PRIMARY KEY (id);

ALTER TABLE ONLY workflow_webhook_events
    ADD CONSTRAINT workflow_webhook_events_workspace_webhook_occurrence_key UNIQUE (workspace_id, webhook_id, occurrence_key);

ALTER TABLE ONLY workflow_webhooks
...
ALTER TABLE ONLY workspace_default_skill_files
    ADD CONSTRAINT workspace_default_skill_files_pkey PRIMARY KEY (default_id, path);

ALTER TABLE ONLY workspace_defaults
    ADD CONSTRAINT workspace_defaults_pkey PRIMARY KEY (id);

ALTER TABLE ONLY workspace_initial_default_skill_files
    ADD CONSTRAINT workspace_initial_default_skill_files_pkey PRIMARY KEY (workspace_id, default_id, path);

ALTER TABLE ONLY workspace_initial_defaults
    ADD CONSTRAINT workspace_initial_defaults_pkey PRIMARY KEY (workspace_id, id);
...
CREATE UNIQUE INDEX generated_documents_conversation_run_tool_call_unique ON generated_documents USING btree (conversation_run_id, tool_call_id) WHERE ((conversation_run_id IS NOT NULL) AND (tool_call_id IS NOT NULL));

CREATE INDEX generated_documents_retention_idx ON generated_documents USING btree (retention_expires_at, id);

CREATE UNIQUE INDEX generated_documents_workflow_run_tool_call_unique ON generated_documents USING btree (workflow_run_id, tool_call_id) WHERE ((workflow_run_id IS NOT NULL) AND (tool_call_id IS NOT NULL));
...
CREATE INDEX sessions_agent_conversations_idx ON sessions USING btree (workspace_id, agent_id, last_message_at DESC, id DESC) WHERE ((conversation_kind = 'agent_chat'::text) AND (deleted_at IS NULL));
...
CREATE UNIQUE INDEX workflow_messages_one_assistant_per_run_idx ON workflow_messages USING btree (run_id) WHERE ((role = 'assistant'::text) AND (run_id IS NOT NULL));
...
CREATE INDEX workflow_webhook_deliveries_claim_idx ON workflow_webhook_deliveries USING btree (next_attempt_at, created_at, id) WHERE (status = ANY (ARRAY['pending'::text, 'failed'::text, 'claimed'::text]));

CREATE INDEX workflow_webhooks_workspace_idx ON workflow_webhooks USING btree (workspace_id, created_at, id);

CREATE INDEX workspace_defaults_available_in_idx ON workspace_defaults USING gin (available_in);

CREATE INDEX workspace_defaults_kind_name_idx ON workspace_defaults USING btree (kind, lower(name), id);

CREATE INDEX workspace_initial_defaults_workspace_kind_idx ON workspace_initial_defaults USING btree (workspace_id, kind, lower(name), id);
...
CREATE TRIGGER admin_audit_events_append_only BEFORE DELETE OR UPDATE ON admin_audit_events FOR EACH ROW EXECUTE FUNCTION prevent_admin_audit_event_mutation();
...
ALTER TABLE ONLY sessions
    ADD CONSTRAINT fk_sessions_linked_issue_target FOREIGN KEY (workspace_id, target_id, linked_issue_id) REFERENCES target_issues(workspace_id, target_id, id) ON DELETE SET NULL (linked_issue_id);
...
ALTER TABLE ONLY generated_documents
    ADD CONSTRAINT generated_documents_conversation_run_id_fkey FOREIGN KEY (conversation_run_id) REFERENCES runs(id) ON DELETE CASCADE;

ALTER TABLE ONLY generated_documents
    ADD CONSTRAINT generated_documents_workflow_execution_id_fkey FOREIGN KEY (workflow_execution_id) REFERENCES workflow_executions(id) ON DELETE CASCADE;

ALTER TABLE ONLY generated_documents
    ADD CONSTRAINT generated_documents_workflow_run_id_fkey FOREIGN KEY (workflow_run_id) REFERENCES workflow_runs(id) ON DELETE CASCADE;

ALTER TABLE ONLY generated_documents
    ADD CONSTRAINT generated_documents_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY runs
    ADD CONSTRAINT runs_workspace_agent_fkey FOREIGN KEY (workspace_id, agent_id) REFERENCES agent_definitions(workspace_id, id) ON DELETE RESTRICT;
...
ALTER TABLE ONLY sessions
    ADD CONSTRAINT sessions_workspace_agent_fkey FOREIGN KEY (workspace_id, agent_id) REFERENCES agent_definitions(workspace_id, id) ON DELETE RESTRICT;
...
ALTER TABLE ONLY workflow_webhook_deliveries
    ADD CONSTRAINT workflow_webhook_deliveries_event_id_fkey FOREIGN KEY (event_id) REFERENCES workflow_webhook_events(id) ON DELETE CASCADE;

ALTER TABLE ONLY workflow_webhook_deliveries
    ADD CONSTRAINT workflow_webhook_deliveries_webhook_id_fkey FOREIGN KEY (webhook_id) REFERENCES workflow_webhooks(id) ON DELETE CASCADE;

ALTER TABLE ONLY workflow_webhook_deliveries
    ADD CONSTRAINT workflow_webhook_deliveries_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE;

ALTER TABLE ONLY workflow_webhook_events
    ADD CONSTRAINT workflow_webhook_events_webhook_id_fkey FOREIGN KEY (webhook_id) REFERENCES workflow_webhooks(id) ON DELETE CASCADE;

ALTER TABLE ONLY workflow_webhook_events
    ADD CONSTRAINT workflow_webhook_events_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE;

ALTER TABLE ONLY workflow_webhooks
    ADD CONSTRAINT workflow_webhooks_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE;

ALTER TABLE ONLY workflow_webhooks
    ADD CONSTRAINT workflow_webhooks_workspace_id_workflow_id_fkey FOREIGN KEY (workspace_id, workflow_id) REFERENCES workflow_definitions(workspace_id, id) ON DELETE RESTRICT;
...
ALTER TABLE ONLY workspace_default_skill_files
    ADD CONSTRAINT workspace_default_skill_files_default_id_fkey FOREIGN KEY (default_id) REFERENCES workspace_defaults(id) ON DELETE CASCADE;

ALTER TABLE ONLY workspace_initial_default_skill_files
    ADD CONSTRAINT workspace_initial_default_skill_fi_workspace_id_default_id_fkey FOREIGN KEY (workspace_id, default_id) REFERENCES workspace_initial_defaults(workspace_id, id) ON DELETE CASCADE;

ALTER TABLE ONLY workspace_initial_defaults
    ADD CONSTRAINT workspace_initial_defaults_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE;
