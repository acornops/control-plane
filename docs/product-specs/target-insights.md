...
Target Insights is a target-scoped built-in assistant tool that improves future troubleshooting by retrieving and learning operational insights. It is not user preference memory, not a skill, not an MCP server, and not vectorized.

## Product Contract

- The platform flag `TARGET_INSIGHTS_ENABLED` controls whether the feature is exposed. It defaults to enabled.
- When the platform flag is enabled, `target_insights` appears in target tools and is enabled by default per target.
- There is no auto-update setting. If the tool is enabled, learning runs automatically when prerequisites are met.
- Insights are scoped to the target. Current target types are Kubernetes clusters and virtual machines.
...
- Reset hard-deletes the target's Target Insights entries and checkpoint jobs. Audit and run history remain intact.

## Data Model

Postgres is the source of truth.

- `target_insights_entries` stores target-scoped Markdown entries plus structured metadata: status, tags, signals, scope, evidence summary, observation count, confidence, first and last observed timestamps, and normal timestamps.
- `target_insights_checkpoint_jobs` stores one durable checkpoint job per workspace, target, and session. Each job records the last observed session activity, due time, processing status, lease, retry state, attempt count, and last error.
- Entry statuses are `active`, `pending`, and `archived`.
- Evidence is stored as concise summaries and normalized signals. Source run IDs are intentionally not stored in insight entries; audit and run history remain the evidence trail.
...
If no entries match, no Target Insights context is injected. If entries match, the control plane injects a compact system context block and returns snippet metadata in the run context. The run context also includes a Target Insights retrieval status so the execution engine can emit a `target_insights_context_retrieved` run event for hits, misses, disabled/skipped retrieval, and retrieval errors.
...
- If AI settings are missing or invalid, the checkpoint is skipped with a Target Insights audit event and marked processed for the current session activity. New session activity or admin changes to AI settings, provider credentials, or Target Insights tool settings can make it eligible again.
...
Target Insights exposes low-cardinality Prometheus metrics through the existing control-plane `/metrics` endpoint. Deployments can scrape these directly or route them through OpenTelemetry Collector into Elastic or another backend.
...
Metrics must not include workspace IDs, target IDs, session IDs, entry IDs, raw prompts, raw logs, or insight text.

## Permissions And Audit

`manage_target_insights` controls mutation. Owner and admin role templates include it by default. Operators with target read access can view entries, activity, settings, and exported content but cannot edit, promote, archive, reset, or change config.

Target Insights activity uses the existing workspace audit system with category `insights`. There is no separate logging mechanism.

Audited events include:

- `target_insights.entry.created.v1`
- `target_insights.entry.updated.v1`
- `target_insights.entry.archived.v1`
- `target_insights.entry.promoted.v1`
...
```ts
interface TargetInsightsToolConfig {
  learning: {
...
- No compatibility adapters for unreleased legacy Target Insights shapes.
