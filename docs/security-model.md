...
- Accept personal MCP PATs only on the current user's installation-scoped
  connection route, forward them once to the gateway, and never place them in
...
- External integration credentials are default-deny except for
  ...
  snapshot resource and durable issue reads, read-only session/message
