...
- A raw external integration client bearer token whose SHA-256 hash appears in
  the control-plane `EXTERNAL_INTEGRATION_CLIENTS_JSON` descriptor.
- A stable external user id, sent as `externalUserId` during linking and as
  `x-acornops-external-user-id` during bot API calls.

The client bearer token is not a browser session, admin token, run token, or
...
these AcornOps workspace capabilities:
...
workspace role. The bot never gets more access than the linked user has.
