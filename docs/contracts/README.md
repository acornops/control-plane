...
- Visible template-origin Agents are system-provided definitions. Workspace managers may change their availability, supported external bindings, duplicate them, or delete them; Agent definition edits and version restore require a duplicated manual draft. Agent deletion reports dependent workflows until those dependencies are removed.
