...
- Workspace provider status identifies `workspace`, `platform_default`, or
  ...
  platform fallback.
- Workspace skill defaults accept either a bounded manual Markdown bundle or a
...
- Protected Admin Audit persists authentication lifecycle events and privileged
  mutation requests, successes, and failures. Read-only admin requests remain
  visible in structured HTTP access logs but do not create audit records.
  Legacy read/search rows remain stored as append-only evidence but are excluded
  from Admin Audit query results.
- Admin Audit keeps `workspaceId` as an exact-match compatibility filter and
  exposes `workspaceQuery` for either an exact workspace ID or a
...
- `permissionMode=read_only|ask_before_changes|auto_allowed_changes` is the
  shared run-safety vocabulary for Agents and targets. Effective policy is the
  most restrictive intersection of the configured Agent or target ceiling,
  conversation/run access, workspace authorization, reviewed tool capability,
  and connector support. Kubernetes cluster responses expose the effective
  mode, optional override, and source. The legacy write-confirmation boolean is
  accepted and projected conservatively for rolling browser compatibility.
...
- Agents and workflows are workspace-owned definitions. Workspace managers may edit, restore, duplicate, disable, or delete them under normal permissions. Agent deletion reports dependent workflows until those dependencies are removed.
