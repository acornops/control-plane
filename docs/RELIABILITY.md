...
- Workflow execution is deterministic and sequential. Each executable step snapshots exactly one active Agent, its version, the Workflow version, tool-route scope, and idempotency key before dispatch. A target is selected only for a target-tool invocation and must match a signed route.
