...
assert(
  baseline.includes('rbac_additions jsonb') &&
    ...
    baseline.includes('rbac_additions_content_hash') &&
    baseline.includes("'kubernetes_rbac_additions'::text"),
  'the greenfield baseline must include Kubernetes RBAC selection and platform-setting state'
);
...
const expectedColumns = [
  ...
  ['agent_definitions', 'target_access_policy'],
...
async function runSqlChecks(databaseUrl) {
  ...
  try {
    ...
    for (const [table, column] of [
      ...
      ['workflow_executions', 'prompt_digest'],
      ['workflow_executions', 'binding_digest'],
      ['workflow_executions', 'resource_bindings'],
      ['workflow_executions', 'resolved_at'],
      ['workflow_definitions', 'enabled_mcp_servers'],
      ['workflow_definitions', 'enabled_skills'],
      ['workflow_runs', 'agent_version'],
      ['workflow_runs', 'prompt_digest'],
      ['workflow_runs', 'binding_digest'],
      ['workflow_runs', 'resource_bindings'],
      ['workflow_runs', 'resolved_at'],
