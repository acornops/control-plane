...
expect(packageJson.name === 'acornops-control-plane', 'package.json name should identify the control plane package');
...
expectIncludes(releaseWorkflow, 'IMAGE_NAME: acornops/control-plane', 'Release workflow image name');
expectIncludes(releaseWorkflow, 'target: runtime', 'Release workflow Docker target');
...
const defaultMaxSourceLines = 550;
const defaultMaxTestLines = 450;
...
for (const file of walkSourceFiles('src')) {
  ...
  expect(
    ...
    `${file} has ${lineCount} lines; budget is ${defaultMaxSourceLines}. Extract a focused controller, repository module, mapper, or helper instead of growing this file.`
  );
}

for (const file of walkSourceFiles('test')) {
  const lineCount = read(file).split('\n').length;
  expect(
    lineCount <= defaultMaxTestLines,
    `${file} has ${lineCount} lines; budget is ${defaultMaxTestLines}. Split shared fixtures and focused test suites instead of growing this file.`
  );
}
